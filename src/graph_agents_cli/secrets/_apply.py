...
def existing_secret_values(name: str, target: Target, keys: list[str]) -> dict[str, str]:
    """Decoded values of the live Secret for ``keys`` (``{}`` when it does not exist)."""
    live = read_live_secret(name, target)
    return {k: live.values[k] for k in keys if k in live.values}
...
class SecretPlan:
    ...
    kept_if_live: list[str] = field(
        default_factory=list
    )  # --dry-run: kept when the Secret has them
    ...
    *,
    ...
    if plan.api_key_rotated:
        console.print(
            ...
            "`graph-agents-cli deploy --env <env> --restart` to use the new API_KEY.",
            style="yellow",
            markup=False,
        )
...
def provision(
    *,
    ...
    """Read the live Secret (not under ``dry_run``), plan, and apply: the shared ``secrets apply`` core."""
    live = None if dry_run else read_live_secret(name, target)
    if live is not None:
        carried = [
            k for k in allowed if k in live.undecodable and k not in select_allowed(values, allowed)
        ]
        if carried:
            raise ConfigError(
