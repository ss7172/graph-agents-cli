...
| `graph-agents-cli-deploy` | Deployment modes, environments, env-file and kube-context rules, rollouts and rollback, secrets and rotation, the Helm chart, Argo CD PR flow, GitHub settings, `infra check`, local-load clusters, disconnected profile |
| `graph-agents-cli-observability` | `TRACING_ENABLED` opt-in, `TRACE_CAPTURE`, LangSmith versus OTLP, hashed principal id, run records and retention, JSON logs, `/metrics`, `/health` and `/ready` |
