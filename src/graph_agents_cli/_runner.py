...
# Set for an extension's command so any re-entry into `graph-agents-cli <cmd>` runs the
# built-in instead of recursing back into the override.
DISABLE_OVERRIDES_ENV = "GRAPH_AGENTS_CLI_DISABLE_OVERRIDES"
# Exported so an extension's scripts can find sibling resources (templates etc.).
EXTENSION_DIR_ENV = "GRAPH_AGENTS_CLI_EXTENSION_DIR"
...
def redact_cmd(args: list[str]) -> str:
    ...
    for i, arg in enumerate(args):
        if arg == "--github-pat" and i + 1 < len(args):
            ...
            secret in arg for secret in ["GITHUB_PAT", "GH_TOKEN", "GITHUB_TOKEN", "GITHUB_APP_KEY"]
        ):
