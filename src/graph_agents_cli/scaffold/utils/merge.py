...
version (``uvx --from <pinned_install_spec(version)> graph-agents-cli``), or by
the exact build a caller names (``old_source``: ``scaffold upgrade
--baseline-ref``, or the commit a manifest records for a build between
releases), because the current templates would misclassify files that changed
between the two. When that build cannot be fetched and run, or
...
from .version import BaselineSource
...
def _run_pinned_create(
    ...
    """Generate the snapshot via ``uvx --from <spec> graph-agents-cli scaffold create``.

    The spec is ``source.spec`` when a build was named (``--baseline-ref``, or
    the commit a manifest records), else ``pinned_install_spec(version)``.
    ...
    spec = source.spec if source is not None else pinned_install_spec(version)
    if spec is None:
        return False
    refresh = ["--refresh-package", PACKAGE_NAME] if source is not None and source.refresh else []
    cmd = [
        "uvx",
        *refresh,
        "--from",
    ...
    label = source.label if source is not None else version

    try:
        result = run_resolved(cmd, capture_output=True, text=True)
    except Exception as e:
        logging.error("Could not run uvx to generate the %s template: %s", label, e)
        return False

    if result.returncode != 0:
        logging.debug("uvx create for %s exited %s:\n%s", label, result.returncode, result.stderr)
        tail = [line for line in (result.stderr or "").splitlines() if line.strip()][-5:]
        logging.error(
            "Failed to generate the %s template via uvx%s",
            label,
...
def run_create_command(
    ...
    A ``source`` (a named build: ``--baseline-ref``, or the commit a manifest
    records) is rendered by that build through ``uvx``, whatever its version.
    Otherwise no ``version``, or the running CLI's own version, renders with the
    bundled templates, and a prior ``version`` is re-created by that exact
    release through ``uvx`` (the authentic baseline). If the build cannot be
    fetched and run (source unreachable, version absent, ``uvx`` missing,
    unparseable version), the snapshot is NOT generated and False is returned,
    so the caller stops with no changes. ``baseline="current"`` is the explicit,
    logged opt-in to render the old snapshot with the current templates instead.
    ...
    if source is not None and baseline == "authentic":
        if _run_pinned_create(args, output_dir, project_name, version or "", source):
            return True
        logging.error(
            "Could not build the %s baseline from %s via uvx (source unreachable, ref "
            "absent, or uvx missing). Nothing was changed.",
            PACKAGE_NAME,
            source.label,
        )
        return False
...
def run_three_way_merge(
    *,
    ...
    baseline: Baseline = "authentic",
    old_source: BaselineSource | None = None,
    auto_approve: bool,
    dry_run: bool,
    prefer_new: bool = False,
    interactive: bool = False,
    ...
    snapshot_check: Callable[[pathlib.Path, pathlib.Path], None] | None = None,
) -> bool:
    ...
    Args:
        ...
        old_source: The build that renders the old snapshot, when one is named
            (it wins over ``old_version``'s release; authentic baseline only).
        ...
        snapshot_check: Called with the old and new snapshot directories once
            both exist, before anything is compared; it may raise a
            ``click.ClickException`` to stop with the project unchanged.
    ...
    """
    same_config = (
        sorted(old_args) == sorted(new_args) and old_version is None and old_source is None
    )
    ...
    try:
        ...
        if same_config:
            ...
            if not run_create_command(
                old_args, old_template_dir, project_name, old_version, baseline, old_source
            ):
                console.print(
        ...
        if snapshot_check is not None:
            snapshot_check(old_template_project, new_template_project)
