...
version (``uvx --from <install_spec(version)> graph-agents-cli``), because the
current templates would misclassify files that changed between the two
versions. When that version cannot be fetched and run, the merge stops with no
changes unless ``baseline="current"`` was passed, an explicit and logged
opt-in to compare against the current templates.
"""
...
def _run_pinned_create(
    ...
    """Generate the snapshot via ``uvx --from <install_spec(version)> graph-agents-cli scaffold create``.

    Returns true if the snapshot was generated, false otherwise.
    """
    from graph_agents_cli._runner import run_resolved

    from .version import PACKAGE_NAME, install_spec
