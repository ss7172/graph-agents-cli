...
| "ask a human before the agent writes", "confirm before cancelling", "human in the loop", "approval" | an `approval` block on the API (`graph-agents-cli api approval NAME --methods POST,DELETE --approvers requester`, or `--operations cancelOrder`): the run pauses before the gated call and an approver decides it (`run` asks on a terminal; `graph-agents-cli approvals approve\|reject`). Approval never widens access |
| "four-eyes", "a second person must approve", "a manager signs off" | `--approvers role:<name>` without `requester` (another principal holding the role decides), under the `jwt` or `custom` auth policy (`shared-bearer` has one principal) |
| "the customer confirms changes but an admin approves new orders", "different approvers for different actions" | a list of approval rules on one API: `graph-agents-cli api approval NAME --add-rule --operations createOrder --approvers role:admin` after the first rule; the first rule in file order that covers a call gates it (`--rule N` changes one). Never a second API entry for the same backend |
| "don't let customer text steer the agent", "prompt injection", "act only on the user's own records" | tool results are fenced as untrusted data (`UntrustedToolResults`) and the default prompt forbids following them; write tools (and reads across customers in staff sessions) call `require_user_mentioned` and `require_owner`, and write APIs use `auth: forward` so the upstream authorizes the user (`/graph-agents-cli-langgraph-code` section 2a) |
...
| "exact case", "in any turn", "across the whole conversation" | `expect.case_insensitive: false` (`contains`/`not_contains` ignore case by default); `expect.scope: all_turns` (checks read every turn, not only the final one) |
